...
function createSocialSVG(name) {
  // iOS Safari compatible SVG with proper namespace and fallbacks
  const isIOS = /iPad|iPhone|iPod/.test(navigator.userAgent);
  
  if (name === 'facebook') {
    if (isIOS) {
      // Fallback for iOS Safari
      return `<span class="social-icon-facebook">f</span>`;
    }
    ...
    if (isIOS) {
      // Fallback for iOS Safari
      return `<span class="social-icon-instagram">ig</span>`;
    }
